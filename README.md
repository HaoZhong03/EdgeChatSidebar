...
1. 打开“拓展选项 → 关于”，点击版本号。
2. 首次使用时允许扩展访问 GitHub；检测到新版本后点击“一键更新”。
3. 在 Edge 弹出的目录选择器中选择当前加载的 `EdgeChatSidebar` 文件夹。扩展会校验并替换程序文件，完成后自动重新加载。

更新不会清除 API Key、设置或历史对话。更新过程中如有文件写入失败，会自动恢复已替换的原文件；同名的本地源码修改仍会被新版本覆盖。若使用 Git 克隆并保留了自己的代码改动，建议先提交或备份改动，或改用 `git pull` 更新，以免工作区与 Git 记录不一致。

当前更新源是 GitHub `main` 分支，不要求单独创建 GitHub Release。发布新版本时需要同步提高 `manifest.json`、`package.json` 和 `package-lock.json` 中的版本号，再将完整代码推送到 `main`。

## ⚙️使用前配置

本程序默认内置了对`DeepSeek`和`小米 MiMo`的配置支持，它们都支持联网搜索；`deepseek-flash` 和 `mimo-v2.5` 还支持图片输入。**如果想用其他的或者自己的模型，也可以自己添加自定义提供商。**

| 提供商 | 模型名称 | 百万 tokens 输出价格 | 备注 |
| --- | --- | --- | --- |
| DeepSeek | `deepseek-flash` | 空闲 4元 / 高峰 8元 | DeepSeek-V4.1-Flash，支持图片输入和联网搜索 |
| DeepSeek | `deepseek-v4-pro` | 空闲 13.5元 / 高峰 27元 | DeepSeek-V4-Pro-0813，支持联网搜索，不支持图片输入 |
...
1. 在“模型 API”区域为 DeepSeek 或小米 MiMo 填写 API Key；也可以添加自定义 OpenAI 格式的 Chat Completions 提供商。
2. 保存后开始对话。

## 📖使用中说明

如果模型配置正确，左下角的模型状态圆点会显示为绿色；未连接时显示为红色。

1. 点击左下角的连接状态圆点展开列表，可手动选择 DeepSeek 或小米 MiMo 的具体模型，默认使用 DeepSeek；列表末尾提供“设置”入口。选择 `deepseek-flash` 或 `mimo-v2.5` 时，可在输入框中直接粘贴 PNG、JPEG 或 WebP 图片并随消息发送。每条消息最多 4 张图片，单张不超过 5MB。
...
3. 可通过模型状态圆点右侧的黄色圆点打开与模型列表同样样式的历史对话菜单，进行切换、压缩上下文或删除操作；右下角的圆形上箭头用于发送消息。
4. 新发送的消息会记录真实发送时间；开启时间戳后，它显示在消息下方。最新用户消息的时间位于编辑按钮左侧。
5. 如果你对模型的回答不满意，可以对最新发送的一条消息进行编辑并重新发送。
6. 可在“拓展选项 → 外观与消息”中调整全局字号、Token 用量显示，设置默认、纯色或自定义图片背景，并统一控制对话输入区与状态栏的不透明度和高斯模糊程度。
...
- 使用 `deepseek-flash` 或小米 MiMo `mimo-v2.5` 粘贴图片发送时，扩展会按官方 OpenAI 兼容多模态消息格式发送：`content: [{ type: "text", text }, { type: "image_url", image_url: { url: "data:image/...;base64,..." } }]`。DeepSeek 联网搜索通道会自动转换为对应的 Anthropic 图片块。图片在磁盘上作为独立加密二进制记录保存，不保留明文 Data URL。
